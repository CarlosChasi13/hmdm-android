...
public class MainActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toast.makeText(this, "Pasó por verificar", Toast.LENGTH_SHORT).show();
        VerificarKiosk();
        hideSystemBars();
    }
  

    private void VerificarKiosk() {
        final ServerConfig config = settingsHelper.getConfig();
        
        if (config != null && config.getKioskMode() != null) {
            boolean kioskConfigured = config.getKioskMode();
            boolean kioskRunning = isKioskModeRunning();
    
            if (kioskConfigured) {
                if (!kioskRunning) {
                    initializeKioskMode(config);
                    Toast.makeText(this, "Modo Kiosko Activado", Toast.LENGTH_LONG).show();
                } else {

                    Toast.makeText(this, "Modo Kiosko ya está activo", Toast.LENGTH_SHORT).show();
                }
            } else {
                if (kioskRunning) {
                    exitKioskMode();
                    ...
                    Toast.makeText(this, "Modo Kiosko está desactivado", Toast.LENGTH_SHORT).show();
                }
            }
        }
    }
    ...
    private void showContent(ServerConfig config) {
        ...
        Toast.makeText(this, "Pasó por Show content", Toast.LENGTH_SHORT).show();
        VerificarKiosk();
        hideSystemBars();
    ...
    private void initializeKioskMode(ServerConfig config) {
            ...
            if (config.getMainApp() != null && !config.getMainApp().isEmpty()) {
                ...
                if (intent != null) {
                    ...
                    new Handler().postDelayed(() -> {
                        startLockTask();
                        hideSystemBars();
                        Toast.makeText(this, "Modo Kiosco activado para " + config.getMainApp(), Toast.LENGTH_LONG).show();
                    }, 500);  
                } else {
    ...
    private boolean isKioskModeRunning() {
    ...
    private void exitKioskMode() {
        stopLockTask();
        Toast.makeText(this, "Saliendo de la aplicación principal", Toast.LENGTH_SHORT).show();

        String mainAppPackage = settingsHelper.getConfig().getMainApp();
        if (mainAppPackage != null && !mainAppPackage.isEmpty()) {
            Intent intent = new Intent(Intent.ACTION_DELETE);
            intent.setData(Uri.parse("package:" + mainAppPackage));
            startActivity(intent);
        }
    }
}
